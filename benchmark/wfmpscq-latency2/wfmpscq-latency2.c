...
int main()
{
	...
	sizeQshm = sizeof(wfmpscq_head_t) + (NUM_PROC - 1) * (sizeof(wfmpscq_queue_d_t) +  NUM_SLOTS * (sizeof(wfmpscq_slot_t) + NODE_DATA_SIZE));
	...
	if (wfmpscq_get(&q, SHMID_QUEUE, NUM_PROC - 1, pID - 1, DQSTR_IDABSOLUTE, NODE_DATA_SIZE, NUM_SLOTS) == -1) {
		printf("Proc %d: wfmpscq_get() failed\n", pID);
		return 0;
	}
	...
	if (pID != 0) {
		...
		for (i = 0; i < NUM_ITERATION; i++) {
			...
			/*	hwfunctions_invalidateCacheRegion(q.mem, sizeQshm);*/
				...
				t1 = read_tsc();
#endif
				while (wfmpscq_enqueue(&q, data) != 0)
					enqueue_fails++;
#ifdef ARCH_X86
				t2 = read_tsc();
#endif
		...
		for (i = 0; i < NUM_ITERATION; i++) {
			...
			while (cnt < NUM_MESSAGES * (NUM_PROC - 1)) {
				...
				t1 = read_tsc();
#endif
				while (wfmpscq_dequeue(&q, data) != 0)
					dequeue_fails++;
#ifdef ARCH_X86
				t2 = read_tsc();
#endif
	...
	if (pID == 0) {
		printf("\nProc %d (dequeue): interations: %lld dequeue fails: %lld t_min: %lld t_max: %lld average %lld \n\n", pID, d, dequeue_fails, t_min, t_max, t_sum / d);
	} else {
		printf("\nProc %d (enqueue): iterations: %lld enqueue fails: %lld t_min: %lld t_max: %lld average %lld\n\n", pID, d, enqueue_fails, t_min, t_max, t_sum / d);
	}
